...
class Logger final : public Singleton<Logger>
{
  ...
  public:
    ...
    struct LogStream : public std::ostringstream {
      public:
        LogStream(const spdlog::source_loc &loc, spdlog::level::level_enum lvl, std::string_view prefix) : mLoc(loc), mLvl(lvl), mPrefix(prefix)
        {}
  ...
  public:
    bool init(std::string_view logFilePath)
    {
        ...
        try
        {
            ...
            // auto dailySink =
            // std::make_shared<spdlog::sinks::daily_file_sink_mt>(logPath.string(),
...
class LoggerNone final : public Singleton<LoggerNone>
{
  ...
  private:
    ...
    LoggerNone &operator<<(const char *content)
    {
        return *this;
    }
};
